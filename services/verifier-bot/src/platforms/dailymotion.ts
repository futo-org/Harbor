...
import { createCookieEnabledAxios, httpResponseToError } from '../utility.js';
...
class DailymotionTextVerifier extends TextVerifier {
  ...
  protected async getText(claimField: ClaimField): Promise<Result<string>> {
    if (
      process.env.POLYCENTRIC_VERIFIER_BOT_DAILYMOTION_CLIENT_ID ===
        undefined ||
      process.env.POLYCENTRIC_VERIFIER_BOT_DAILYMOTION_CLIENT_SECRET ===
        undefined
    ) {
    ...
    const anonymousTokenResult = await client({
      ...
      data: {
        ...
        client_id: process.env.POLYCENTRIC_VERIFIER_BOT_DAILYMOTION_CLIENT_ID,
        client_secret:
          process.env.POLYCENTRIC_VERIFIER_BOT_DAILYMOTION_CLIENT_SECRET,
      },
    });
