...
pub(crate) fn set_defaults(config: &mut ClientConfig) {
    ...
    if let Ok(value) = env::var("HARBOR_KAFKA_SSL_CA") {
        config.set("ssl.ca.pem", value);
    }
    if let Ok(value) = env::var("HARBOR_KAFKA_SSL_CERTIFICATE") {
        config.set("ssl.certificate.pem", value);
    }
    if let Ok(value) = env::var("HARBOR_KAFKA_SSL_KEY") {
        config.set("ssl.key.pem", value);
    }

    config.set(
        "broker.address.family",
        env::var("HARBOR_KAFKA_BROKER_ADDRESS_FAMILY").unwrap_or_else(|_| "any".to_string()),
    );
}

/// Prefix `name` with `HARBOR_KAFKA_CLUSTER_ID` (as `{id}.{name}`) so
/// multiple clusters can share one broker. No prefix when unset or empty.
pub fn prefixed(name: &str) -> String {
    match env::var("HARBOR_KAFKA_CLUSTER_ID") {
...
pub(crate) fn auto_offset_reset() -> String {
    env::var("HARBOR_KAFKA_AUTO_OFFSET_RESET").unwrap_or_else(|_| "latest".to_string())
}
...
mod tests {
    ...
    fn prefixed_applies_cluster_id() {
        ...
        unsafe { env::set_var("HARBOR_KAFKA_CLUSTER_ID", "staging") };
        assert_eq!(prefixed("events"), "staging.events");

        unsafe { env::set_var("HARBOR_KAFKA_CLUSTER_ID", "") };
        assert_eq!(prefixed("events"), "events");

        unsafe { env::remove_var("HARBOR_KAFKA_CLUSTER_ID") };
        assert_eq!(prefixed("events"), "events");
    }
}
