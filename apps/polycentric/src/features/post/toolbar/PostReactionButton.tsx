...
function PostReactionButton({ post }: PostReactionButtonProps) {
    ...
    const onEmojiSelect = async (emoji: string) => {
        ...
        if (hasReaction) {
            ...
            if (reaction.emoji === emoji)
                return; // Same emoji means we removed it
        }
    ...
    return <View style={[]}><HoverCard openDelay={0} onOpenChange={setOpen}>
        <HoverCard.Trigger asChild ref={triggerRef} onPress={onReactionPress} onLongPress={triggerRef.current?.open}>
            <PostActionButton
