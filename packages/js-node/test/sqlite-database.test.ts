...
describe('OPFSSQLiteDatabase', () => {
  ...
  beforeEach(async () => {
    ...
    db = await _createNodeSQLiteDatabase(dbName, TEST_DB_DIR, testSchema);
    await db.initialize();
  ...
  test('should throw error for empty database name', async () => {
    await expect(
      async () => await _createNodeSQLiteDatabase(''),
  ...
  test('should throw DatabaseError when operating on uninitialized database', async () => {
    const uninitializedDb = await _createNodeSQLiteDatabase(
      'uninitialized',
      TEST_DB_DIR,
      testSchema,
    );
  ...
  test('should use custom schema', async () => {
    ...
    const customDb = await _createNodeSQLiteDatabase(
      `custom-${Date.now()}`,
      TEST_DB_DIR,
      customSchema,
    );
